...
@section('title', 'Add Charge Template')
...
<div class="row">
    <form action="{{ route('charge_templates.store') }}" method="POST" onkeydown="return event.key != 'Enter';">
        @csrf
        <div class="col-12 mb-2">
            <section class="card">
                <header class="card-header">
                    <h2 class="card-title">Add Charge Template</h2>
                    @if ($errors->any())
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="name" class="form-control" placeholder="e.g. Catalyst Visa 22/07/2026" value="{{ old('name') }}" required>
                        </div>
                        <div class="col-md-3 mb-3">
                            <label>Service Category<span class="text-danger">*</span></label>
                            <select name="service_category" class="form-control" required>
                                @foreach($categories as $cat)<option value="{{ $cat }}" @selected(old('service_category') === $cat)>{{ ucfirst($cat) }}</option>@endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-1 mb-3">
                            ...
                            <input type="text" name="default_currency" class="form-control" value="{{ old('default_currency', 'PKR') }}" maxlength="3">
                        </div>
                        <div class="col-md-2 mb-3">
                            <label>Default Exch. Rate</label>
                            <input type="number" step="any" name="default_exchange_rate" class="form-control" value="{{ old('default_exchange_rate', 1) }}">
                        </div>
                    </div>
                </div>
            </section>
                                ...
                                <td>
                                    <select name="items[0][charge_type_id]" class="form-control select2-js">
                                        ...
                                        @foreach($chargeTypes as $ct)
                                            <option value="{{ $ct->id }}" data-default="{{ $ct->default_value }}">{{ $ct->name }} ({{ $ct->code }})</option>
                                        @endforeach
                                    </select>
                                </td>
